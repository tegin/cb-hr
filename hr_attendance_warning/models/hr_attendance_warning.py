...
class HrAttendanceWarning(models.Model):
    ...
    solved_by = fields.Many2one("res.users", string="Solved by", readonly=True)
    solved_on = fields.Datetime(string="Solved on", readonly=True)

    solver_comment = fields.Text(
        string="Comments", states={"pending": [("readonly", False)]}
    )

    warning_line_ids = fields.One2many(
        "hr.attendance.warning.line", inverse_name="warning_id"
    )

    day_date = fields.Date(
        string="Creation Date", compute="_compute_day_date", readonly=True
    )

    message_preview = fields.Char(
        compute="_compute_message_preview", readonly=True
    )
    ...
    def _compute_day_date(self):
        for record in self:
            record.day_date = record.create_date.date()

    @api.model
    def get_name(self, vals):
        return (
            self.env["ir.sequence"].next_by_code("hr.attendance.warning")
            or "/"
        )
...
class HrAttendanceWarningLine(models.Model):
    ...
    _description = "Attendance Warning Line"

    warning_id = fields.Many2one("hr.attendance.warning", readonly=True)
    employee_id = fields.Many2one(
        "hr.employee", readonly=True, related="warning_id.employee_id"
    )
    state = fields.Selection(
        [("pending", "Pending"), ("solved", "Solved")],
        readonly=True,
    ...
    message = fields.Char(
        string="Message", compute="_compute_message", readonly=True
    )
